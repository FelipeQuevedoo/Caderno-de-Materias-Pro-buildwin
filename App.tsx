...
function NotebookEditor({
  ...
  return (
    <div data-no-page-turn="true">
      {showToolbar && (
        <div className="toolbar">
          <button onClick={() => editor.chain().focus().toggleBold().run()} title="Negrito"><Bold size={15} /></button>
          <button onClick={() => editor.chain().focus().toggleItalic().run()} title="Italico"><Italic size={15} /></button>
          <button onClick={() => editor.chain().focus().toggleUnderline().run()} title="Sublinhado"><Underline size={15} /></button>
          <button onClick={() => editor.chain().focus().toggleStrike().run()} title="Tachado"><span className="strike-a">A</span></button>
          <button onClick={() => editor.chain().focus().toggleMark("doubleStrike").run()} title="Tachado duplo"><span className="double-strike-a">A</span></button>
          <button onClick={() => editor.chain().focus().toggleSubscript().run()} title="Subscrito"><Subscript size={15} /></button>
          <button onClick={() => editor.chain().focus().toggleSuperscript().run()} title="Sobrescrito"><Superscript size={15} /></button>

          <span className="tool-divider" />

          <button onClick={() => editor.chain().focus().setTextAlign("left").run()} title="Alinhar esquerda"><AlignLeft size={15} /></button>
          <button onClick={() => editor.chain().focus().setTextAlign("center").run()} title="Alinhar centro"><AlignCenter size={15} /></button>
          <button onClick={() => editor.chain().focus().setTextAlign("right").run()} title="Alinhar direita"><AlignRight size={15} /></button>
          <button onClick={() => editor.chain().focus().setTextAlign("justify").run()} title="Justificar"><AlignJustify size={15} /></button>
          <button onClick={() => editor.chain().focus().toggleBulletList().run()} title="Lista nao ordenada"><List size={15} /></button>
          <button onClick={() => editor.chain().focus().toggleOrderedList().run()} title="Lista ordenada"><ListOrdered size={15} /></button>
          <button onClick={() => applyIndent(1)} title="Aumentar recuo"><Indent size={15} /></button>
          <button onClick={() => applyIndent(-1)} title="Diminuir recuo"><Indent size={15} className="indent-flip" /></button>
          ...
          <button className="highlight-tool" onClick={() => highlightInputRef.current?.click()} title="Marca-texto">
          ...
          <button
            onClick={() => {
              const char = window.prompt("Digite o caractere especial", "§");
              if (char) {
          ...
          <button onClick={() => fileInputRef.current?.click()} title="Inserir imagem"><ImagePlus size={15} /></button>
          <input ref={fileInputRef} type="file" accept=".jpg,.jpeg,.bmp,.png,.gif,image/*" onChange={insertImage} className="hidden" />

          <button onClick={() => setTableOpen((prev) => !prev)} title="Inserir tabela"><Table2 size={15} /></button>
          {tableOpen && (
          ...
          <button onClick={() => editor.chain().focus().addRowAfter().run()} title="Inserir linha"><span>+L</span></button>
          <button onClick={() => editor.chain().focus().deleteRow().run()} title="Excluir linha"><span>-L</span></button>
          <button onClick={() => editor.chain().focus().addColumnAfter().run()} title="Inserir coluna"><span>+C</span></button>
          <button onClick={() => editor.chain().focus().deleteColumn().run()} title="Excluir coluna"><span>-C</span></button>
          <button onClick={() => editor.chain().focus().toggleHeaderCell().run()} title="Alternar borda/header"><span>Bd</span></button>

          <button onClick={() => editor.chain().focus().undo().run()} title="Desfazer"><Undo2 size={15} /></button>
          <button onClick={() => editor.chain().focus().redo().run()} title="Refazer"><Redo2 size={15} /></button>
        </div>
      )}
              ...
              >
                ...
                {activePage.kind === "subjectPage" && currentSubject && currentNotebookPage && (
                  <div className="subject-page-sheet" data-no-page-turn="true">
                    ...
                    >
                      <NotebookEditor key={currentNotebookPage.id} page={currentNotebookPage} showToolbar={notebook.settings.showToolbar} onChange={updateFromEditor} onPageFormatChange={updateCurrentPageStyle} />
                    </div>
